<<<<<<< HEAD:_posts/2016-12-27-win10 uwp MVVM入门.md
---
layout: post
...
MVVM 是一个强大的框架，从 WPF 开始，wr（我说的就是微软）就提倡使用 MVVM。它可以将界面和后台分离，让开发人员可以不必关心界面的逻辑，全心投入到后台代码编写中。

*本文主要：如何在 UWP 程序中使用 MVVM 框架，如何做一个自己的框架*
...
MVVM 是 Model、View、ViewModel 合起来的称呼。
- View 就是界面。在 UWP 中我们看到的，一般是 `Page`，`UserControl` 等。
我们写界面时用 xaml 和 cs 结合起来，做出好看的效果。
- ViewModel 是界面的抽象。这里并不知道界面有什么，但是知道可以提供给界面什么。这就是说我们可以不管界面而将业务逻辑抽象出来。ViewModel 是不知道 View 的存在的。ViewModel 可以简单单元测试而不需要打开界面。
- Model 是核心逻辑，有些大神说，Model 只定义数据结构，有些大神说 M
Model 写核心逻辑，这个就仁者见仁智者见智了。我是将核心逻辑写进 Model，如果觉得这样不对，欢迎讨论。

怎么让 ViewModel 抽象 View，随后简单地把界面联系起来呢？  
使用 Binding 即可，这是 WPF 强大的地方，而 UWP 继承并发扬了这些特性。

![MVVM 框架图](http://7xqpl8.com1.z0.glb.clouddn.com/2639f44f-463b-4fd1-b9e9-c01652649f28201612268535.jpg)

想知道如何写 MVVM？戳此链接：[http://www.cnblogs.com/indream/p/3602348.html](http://www.cnblogs.com/indream/p/3602348.html)。
...
>>>>>>> 5dac41a11c72888e4a8785a02f8d32cffcc0e543:_posts/2016-12-26-win10 uwp MVVM入门.md
