---
title: "2018-9-29-Roslyn-通过-Nuget-引用源代码-在-VS-智能提示正常但是无法编译"
author: lindexi
...
这个问题是 msbuild 的一个坑，主要是没有主动引用 `.nuget.g.props` 和 `.nuget.g.targets` 文件，使用 Microsoft.NET.Sdk 作为 Sdk 的项目文件会自动在 obj 文件夹下生成 project.assets.json、$(ProjectName).csproj.nuget.cache、$(ProjectName).csproj.nuget.g.props 和 $(ProjectName).csproj.nuget.g.targets 文件；其中 .nuget.g.props 和 .nuget.g.targets 中生成了 Import 包中编译相关文件的代码。具体请看[MSBuil---
title: "Roslyn 通过 Nuget 引用源代码 在 VS 智能提示正常但是无法编译"
author: lindexi
date: 2018-9-29 12:58:16 +0800
CreateTime: 2018-9-29 12:47:45 +0800
